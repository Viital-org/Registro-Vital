...
@section('conteudo')
    ...
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h4">Áreas de atuação</h1>
            <a href="{{ route('atuareas-create') }}" class="btn btn-primary">
        ...
        <form action="{{ route('atuareas-show') }}" method="post" class="d-flex w-100">
            @csrf
            <input class="form-control me-2 flex-grow-1" type="search" name="busca_area" placeholder="Ex: Derm"
                   aria-label="Search">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover align-middle">
                ...
                @foreach($atuaareas as $item)
                    <tr>
                        <td>{{$item->descricao_area}}</td>
                        <td>
                            @if($item->especializacoes->isNotEmpty())
                                {{ $item->especializacoes->pluck('descricao_especializacao')->implode(', ') }}
                            @else
                                Não definido
                            @endif
                        </td>
