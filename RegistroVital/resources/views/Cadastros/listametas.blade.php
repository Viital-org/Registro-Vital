...
@section('titulo', 'Listagem de Metas')
...
@section('conteudo')
    ...
    <table class="table">
        ...
        <tr>
            ...
            <th scope="col">Pacientes com esta Meta</th>
        ...
        @foreach($metas as $item)
            <tr>
                ...
                <td>{{ $item->meta }}</td>
                <td>
                    @if($item->pacientes->isNotEmpty())
                        {{ $item->pacientes->pluck('nome')->implode(', ') }}
                    @else
                        Não definido
                    @endif
                </td>
                ...
                <td>
                    <a href="{{ route('metas-edit', ['id' => $item->id]) }}">
                ...
                <td>
                    ...
                    <form action="{{ route('metas-delete', ['id'=> $item->id])}}" method="POST">
                        @csrf
                        @method('DELETE')
                        <div class="modal fade" id="delete{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel{{ $item->id }}" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <p>Deseja realmente excluir a meta '{{ $item->meta }}'?</p>
                                    </div>
